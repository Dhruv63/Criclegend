...
class WagonWheelStatsWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final distribution = <String, int>{
      'Long Off': 0, 'Long On': 0, 'Deep Mid Wicket': 0, 'Deep Square Leg': 0,
      'Fine Leg': 0, 'Third Man': 0, 'Deep Point': 0, 'Deep Cover': 0
    };
    ...
    return Column(
      ...
      children: [
        const Text("Wagon Wheel", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        ...
        Text("Total Runs on Wheel: $totalRunsMapped", style: const TextStyle(color: Colors.grey)),
      ],
    );
  }
}
...
class _WagonPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        // Draw Text Labels
        final labelAngle = startAngle + (i * anglePerSector) + (anglePerSector/2);
        ...
        final textSpan = TextSpan(
          text: '$runs',
          style: TextStyle(color: runs > 0 ? Colors.black : Colors.black26, fontWeight: FontWeight.bold, fontSize: 14),
        );
